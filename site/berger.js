...
function drawRRMapRound(table, rid, x = 0, y = 0, r = 1, rnode = 1, pc = null)
{
	...
	for (let pid in data)
	{
		...
		if (!d.white) continue;
		html += `<line data-wid="${pid}" data-bid="${d.opp}" x1="${d.p.x}" y1="${d.p.y}" x2="${d.n.x}" y2="${d.n.y}"></line>`;
	...
	for (let pid in data)
	{
		const d = data[pid];
		if (d.drop) continue;
		html += `<circle data-pid="${pid}" r="${rnode}" cx="${d.p.x}" cy="${d.p.y}"></circle>`;
...
export function renderMapSvg(table, cols = 1, r = 60, rnode = 10, standalone = false)
{
	...
	html += '>';
	html += '<style>text { color: red; pointer-events: none; }</style>';
...
export function renderMapToImg(table, cols = 1, r = 60, rnode = 10)
{
	const map = renderMapSvg(table, cols, r, rnode, true);
...
export function renderMap(table, cols = 1, r = 60, rnode = 10)
{
	// TODO: Move the details into renderMapSvg.
	...
	html += "circle { fill: white; stroke: black; } ";
	html += "line { fill: none; stroke: black; marker-end: url(#end); } ";
	html += "marker { stroke: none; overflow: hidden; } ";
	html += ".cursor { fill: white; stroke: grey; stroke-width: 2; } ";
	html += ".link { fill: none; stroke: grey; stroke-width: 2; } ";
	html += ".hook { fill: white; stroke: grey; } ";
	html += ".ghost { fill: white; stroke: lightgrey; stroke-width: 2; } ";
	html += ".trace { fill: none; stroke: lightgrey; stroke-width: 1; }";
	html += "</style>";
